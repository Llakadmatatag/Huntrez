document.addEventListener('DOMContentLoaded', function() {
    // Set the end date for the countdown (July 29, 2025 23:59:59 UTC)
    const endDate = new Date('2025-07-29T23:59:59Z');
    ...
    function updatePodium(players) {
        ...
        podiumPlayers.forEach(podium => {
            ...
            if (nameEl) nameEl.textContent = player.name || 'Anonymous';
    ...
    const mockPlayers = [
        { name: 'Player 1', wagered: 50000, prize: 500, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 2', wagered: 45000, prize: 300, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 3', wagered: 40000, prize: 200, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 4', wagered: 35000, prize: 0, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 5', wagered: 30000, prize: 0, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 6', wagered: 25000, prize: 0, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 7', wagered: 20000, prize: 0, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 8', wagered: 15000, prize: 0, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 9', wagered: 10000, prize: 0, avatar: 'images/avatar-placeholder.png' },
        { name: 'Player 10', wagered: 5000, prize: 0, avatar: 'images/avatar-placeholder.png' }
    ];
        ...
        try {
            ...
            if (responseData && responseData.error === false && responseData.data && Array.isArray(responseData.data.summarizedBets)) {
                ...
                const processedData = responseData.data.summarizedBets.map((entry, index) => {
                    ...
                    const username = user.displayName || user.username || `Player ${index + 1}`;
                    const avatar = user.avatar || `images/avatar-placeholder.png`;
                    
                    // Calculate total wagered amount (using wager from the entry)
                    const totalWagered = parseFloat(entry.wager) || 0;
                    
                    // Calculate prize based on rank
                    let prize = 0;
                    if (index === 0) prize = 500;        // 1st place: $500
                    else if (index === 1) prize = 250;    // 2nd place: $250
                    else if (index === 2) prize = 100;    // 3rd place: $100
                    else if (index === 3) prize = 50;     // 4th place: $50
                    else if (index === 4) prize = 50;     // 5th place: $50
                    else if (index === 5) prize = 25;     // 6th place: $25
                    else if (index === 6) prize = 25;     // 7th place: $25
                    // 8th place and below get no prize (prize = 0)
    ...
    function loadMockData() {
        ...
        populateLeaderboard(mockData);
        updateLastUpdated(true); // Mark as using mock data
    }
